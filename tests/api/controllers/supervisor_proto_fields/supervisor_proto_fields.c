...
static void retrieve_fields(const char *main_field_names[NUMBER_OF_MAIN_FIELDS][NUMBER_OF_HIERARCHY_LEVELS],
    ...
    WbNodeRef hierarchy, WbFieldRef actual_main_parameters[NUMBER_OF_MAIN_FIELDS],
...
int main(int argc, char **argv) {
  ...
  retrieve_fields(main_field_names, internal_field_names, hierarchy, actual_main_parameters,
                  actual_internal_parameters, main_fields, internal_fields);
  ...
  for (int i = 0; i < NUMBER_OF_MAIN_FIELDS; i++) {
    ...
    for (int j = 0; j < NUMBER_OF_HIERARCHY_LEVELS; j++)
      if (main_fields[i][j])
        check_field(main_fields[i][j], actual_main_parameters[i], actual_main_parameters[i] ? i : 0.01, false, main_field_names[i][j], "Update field [main field]");
  }
  ...
  for (int i = 0; i < NUMBER_OF_INTERNAL_FIELDS; i++) {
    ...
    for (int j = 0; j < NUMBER_OF_HIERARCHY_LEVELS; j++)
      if (internal_fields[i][j])
        check_field(internal_fields[i][j], actual_internal_parameters[i], actual_internal_parameters[i] ? i : 0.01, false, internal_field_names[i][j], "Update field [internal field]");
  }
