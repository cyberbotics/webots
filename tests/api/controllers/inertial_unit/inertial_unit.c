...
int main(int argc, char **argv) {
  ...
  int lookup_table_size = wb_distance_sensor_get_lookup_table_size(inertial_unit);
  ts_assert_double_equal(lookup_table_size, 2, "Lookup table size returned is wrong (%d instead of 2)", lookup_table_size);
  const double *lookup_table = wb_distance_sensor_get_lookup_table(inertial_unit);
