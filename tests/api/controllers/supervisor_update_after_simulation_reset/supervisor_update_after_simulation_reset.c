...
int main(int argc, char **argv) {
  ...
  ts_assert_int_equal(wb_supervisor_field_get_count(root_children), field_count_before_reset - 1, "Root children field count not correctly updated after reset.");
  ts_assert_pointer_not_null(wb_supervisor_field_get_mf_node(root_children, -1), "Root children field index -1 not correctly translated to a valid index.");
