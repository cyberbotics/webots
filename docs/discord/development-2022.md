...
##### Simon Steinmann [Moderator] 02/22/2022 17:57:46
...
##### Olivier Michel [Cyberbotics] 03/09/2022 08:52:28
...
##### Bayes [Moderator] 05/18/2022 17:45:26
...
##### starscream 08/20/2022 09:35:50
Has anyone tried to create grid map by parsing world .wbt file?


Has anyone tried to create grid map by parsing world .wbt file?

##### Arne 08/27/2022 12:43:40
Hello, i want to use the openvr interface in a robot controller and the include works fine, but when i try to call the initVr Function the controller crashs without an error. Has anyone an idea what i can do?

##### JohnR 08/28/2022 11:33:43
Hello everyone. I was wondering if anyone could give any ideas on ways robots can communicate with each other via sensors/ bluetooth or other means that is possible to simulate in webots?

##### DDaniel [Cyberbotics] 08/28/2022 11:40:34
Inter-robot communication can be done using emitter/receivers. There's several sample worlds included in webots you can look at for inspiration under `file > open sample world`. For this specifically you can search for `emitter_receiver.wbt`

## September

##### Einar 09/14/2022 08:48:28
Hi!



I am trying to control a robot through RabbitMQ (Messaging system).



The simulation runs, but it seems like messages are only received when I stop the simulation.
...
The print statement in "callback" is only printed in the Webots console AFTER the simulation is stopped and reset.



The print statement in "controller" is never printed in the Webots console.



Is there something I am missing or can anyone point me in the right direction?

##### Simon Steinmann [Moderator] 09/20/2022 16:41:07
If you are using the webots ROS controller, I think it generates an urdf and uses that. Without your adjustments. You gotta make sure that the urdf export delivers what you want. But I have not been active in a year or so. Best to ask the devs. But consider that tf is a tree, so it only generates everything downward. World and table are probably not children of your robot. Static tf you can always manually publish in your controller. might be an easier workaround
