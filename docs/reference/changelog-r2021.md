...
Released on June, Xth, 2021.
  ...
  - Bug fixes:
    ...
    - **Changed ROS data type of [/supervisor/node/get_type_name](supervisor.md?tab-language=ros#wb_supervisor_node_get_type_name) service that now uses `webots_ros::node_get_name` instead of `webots_ros::node_get_type_name` ([#3202](https://github.com/cyberbotics/webots/pull/3202))**.
  - Cleanup
