...
Released on XXX.
  ...
  - Bug fixes
    - Windows: Fixed JPEG texture errors when typing `webots` from a DOS console (`cmd.exe`) by renaming `webots.exe` to `webots-bin.exe` and creating a launcher named `webots.exe`.
