...
If the given path is accessible by the controller, the sound file is loaded by the controller and streamed to Webots.
Otherwise the given path is passed to Webots. If this path is relative, it will be searched first relatively to the robot controller folder.
