        ...
        text = text.replace(/%robot\s+([^ \n]+)\s+([^ \n]+)/gi, function(match, robot, imageFallback) {
          ...
          if (location.href.startsWith('file:')) {
            ...
            replacement =
              ...
              '        <button class="reset-button" title="fullscreen2." onclick="toogleFullscreen(\'%ROBOT%\')"></button>\n' +
              '        <button class="reset-button" title="Reset Viewpoint 2 and sliders." onclick="resetRobotComponent(\'%ROBOT%\')"></button>\n' +
