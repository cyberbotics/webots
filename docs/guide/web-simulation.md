...
If you have installed Webots with the `.deb` or the `.snap` package, you should:
 ...
 - Open the [default.json]({{ url.github_tree }}/resources/web/server/config/simulation/config.json) file located in [server/config/simulation/]({{ url.github_tree }}/resources/web/server/config/simulation) with your IDE or text editor and add the following line between the `{}`: ``"webotsHome": "/usr/local/webots"``.
