...
The force is computed as follow: *F* [N] = `WorldInfo.dragForceScale` * `Solid.mass` * *d*, where *d* corresponds to the vector created by dragging the mouse (in meters).
...
The object is prompted to turn around the vector direction, the torque is computed as follow: *T* [Nm] = `WorldInfo.dragTorqueScale` * `Solid.mass` * *d*, where *d* corresponds to the vector created by dragging the mouse (in meters).
