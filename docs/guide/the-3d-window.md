...
The force is computed as follow: *F* = `Worldinfo.dragForceScale` * `Solid.mass` * *d* [N], where *d* corresponds to the vector created by dragging the mouse (in meters).
...
The object is prompted to turn around the vector direction, the torque is computed as follow: *T* = `Worldinfo.dragTorqueScale` * `Solid.mass` * *d* [Nm], where *d* corresponds to the vector created by dragging the mouse (in meters).
