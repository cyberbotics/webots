...
If the `Extra project path` parameter and the `WEBOTS_EXTRA_PROJECT_PATH` environment variable are set, then both will be considered, having extra priority the paths set in the `Preferences` setting.
