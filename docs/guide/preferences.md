...
- The **Texture Quality** option allows you to reduce the resolution of the textures in order to use less GPU memory. If set to `high` the maximum available resolution will be used. If set to `medium`, the resolution of images bigger or equal to 1024 is divided by 2 (width and height taken into account independently). If set to `low`, the resolution of images bigger or equal to 512 is divided by 4.
