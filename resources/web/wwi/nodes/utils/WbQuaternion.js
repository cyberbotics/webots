...
export default class WbQuaternion {
  ...
  mul(q) {
    return new WbQuaternion(this.w * q.w - this.x * q.x - this.y * q.y - this.z * q.z,
      this.w * q.x + this.x * q.w + this.y * q.z - this.z * q.y, this.w * q.y + this.y * q.w + this.z * q.x - this.x * q.z,
      this.w * q.z + this.z * q.w + this.x * q.y - this.y * q.x);
  }

  mulByVec3(v) {
    const twoX = 2.0 * this.x;
    const twoY = 2.0 * this.y;
    const twoZ = 2.0 * this.z;
    const sX = twoX * this.x;
    const sY = twoY * this.y;
    const sZ = twoZ * this.z;
    const sW = 2.0 * this.w * this.w;
  ...
  normalize() {
    ...
    if (Math.abs(1.0 - d) < 2.107342e-08) // 2.107342e-08 magic number (> ULP/2 for IEEE doubles)
