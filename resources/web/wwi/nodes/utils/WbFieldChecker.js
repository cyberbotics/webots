...
function resetVector2IfNonPositive(value, defaultValue) {
  if (value.x <= 0 || value.y <= 0) {
    console.warn('Invalid (' + value.x + ' ' + value.y + ') changed to (' + defaultValue.x + ' ' + defaultValue.y + `). The
...
function resetVector3IfNonPositive(value, defaultValue) {
  if (value.x <= 0 || value.y <= 0 || value.z <= 0) {
    console.warn('Box: Invalid "size" changed to ' + defaultValue.x + ' ' + defaultValue.y + ' ' + defaultValue.z + `. The
