  ...
  switch (mode) {
    case MAC:
      ...
      fprintf(
        ...
        "375 100 --icon-size 72 --icon \"Webots.app\" 200 190 --app-drop-link 480 580 \"%s-%s.dmg\" \"%s/%s/\"",
        application_name, webots_home, application_name_lowercase_and_dashes, package_version, distribution_path, bundle_name);
      // --volicon \"application\\_icon.icns\"
      // --hide-extension \"Application.app\"

      // fprintf(fd, "cd %s/\n", distribution_path);
      // fprintf(fd, "mkdir -p /tmp/empty\n");
      // fprintf(fd, "hdiutil create -attach -fs HFS+ -srcfolder /tmp/empty -format UDRW -volname \"%s\" -size 3000m
      // %s.dmg\n",
