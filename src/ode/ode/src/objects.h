...
struct dObject : public dBase {
    dxWorld* old_world; // used for clustering
    ...
    explicit dObject(dxWorld *w): old_world(NULL), world(w), next(NULL), tome(NULL), tag(0), userdata(NULL) {}
    virtual ~dObject();
};
...
struct dxBody : public dObject {
    ...
    bool mtTag;  // used in clustering algorithm to indicate that the body was reattached
...
struct dxWorld : public dBase {
    ...
    dxFluid *firstfluid;  // fluid linked list
    dxJoint *firstjoint;		// joint linked list
    int nb,nj,nf;			// number of bodies, joints, and fluids in lists
    dVector3 gravity;		// gravity vector (m/s/s)
    dReal global_erp;		// global error reduction parameter
    dReal global_cfm;		// global constraint force mixing parameter
    dxAutoDisable adis;		// auto-disable parameters
    int body_flags, fluid_flags; // flags for new bodies and fluids
