...
#include "objects.h"
#include "util.h"
#include "fluid_dynamics/immersion_kernel.h"
...
{
#ifdef ODE_MT
    dInitODE_MT(&dInitODE_ST);
#else
    dInitODE_ST();
#endif
}

int dInitODE2_ST(unsigned int uiInitFlags/*=0*/)
{
    bool bResult = false;
...
{
#ifdef ODE_MT
    return dInitODE2_MT(uiInitFlags, &dInitODE2_ST);
#else
    return dInitODE2_ST(uiInitFlags);
#endif
}
