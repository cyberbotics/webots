...
dxJointHinge2::getInfo1( dxJoint::Info1 *info )
{
    ...
    {
        dReal angle1 = measureAngle1();
        dReal angle2 = measureAngle2();
        if ( limiting1 )
...
dxJointHinge2::getInfo2( dReal worldFPS, dReal worldERP, const Info2Descr *info )
{
    ...
    dVector3 ax1, ax2;
    dVector3 susp_ax;  // length 1 suspension axis in global coordinates, from 1st body
    dMultiply0_331( susp_ax, node[0].body->posr.R, susp_axis );
    joint->getAxisInfo( ax1, ax2, q, s, c );
    dNormalize3( q );   // @@@ quicker: divide q by s ?

    // set the three ball-and-socket rows (aligned to the suspension axis susp_ax)
    setBall2( this, worldFPS, worldERP, info, anchor1, anchor2, susp_ax, susp_erp );
...
void dJointSetHinge2SuspensionAxis( dJointID j, dReal x, dReal y, dReal z )
{
    dxJointHinge2* joint = ( dxJointHinge2* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, Hinge2 );
    setAxes( joint, x, y, z, joint->susp_axis, NULL );
}
...
void dJointGetHinge2SuspensionAxis( dJointID j, dVector3 result )
{
    dxJointHinge2* joint = ( dxJointHinge2* )j;
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( result, "bad result argument" );
    checktype( joint, Hinge2 );
    getAxis( joint, result, joint->susp_axis );
}
...
void dJointGetHinge2Axis1( dJointID j, dVector3 result )
{
    ...
    {
      // this code should never be executed (just here to prevent a gcc warning):
      dAASSERT( false );
      result[0] = 0.0;
      result[1] = 0.0;
      result[2] = 0.0;
