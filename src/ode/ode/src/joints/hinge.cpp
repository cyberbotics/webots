...
void dxJointHinge::getInfo2( dReal worldFPS, dReal worldERP, const Info2Descr* info )
{
    ...
    // set the three ball-and-socket rows (aligned to the suspension axis susp_axis)
...
void dJointSetHingeSuspensionAxis( dJointID j, dReal x, dReal y, dReal z )
{
    dxJointHinge* joint = ( dxJointHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, Hinge );
...
void dJointGetHingeSuspensionAxis( dJointID j, dVector3 result )
{
    dxJointHinge* joint = ( dxJointHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( result, "bad result argument" );
