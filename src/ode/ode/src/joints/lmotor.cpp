...
dxJointLMotor::getInfo1( dxJoint::Info1 *info )
{
    ...
    for ( int i = 0; i < num; i++ ) {
        ...
        if (( limot[i].lostop > -dInfinity || limot[i].histop < dInfinity ) &&
            limot[i].lostop <= limot[i].histop) {
            // measure joint position
            dReal pos = dJointGetLMotorPosition ( this, i );
            if ( pos <= limot[i].lostop ) {
                limot[i].limit = 1;
                limot[i].limit_err = pos - limot[i].lostop;
                info->m++;
            } else if ( pos >= limot[i].histop ) {
                limot[i].limit = 2;
                limot[i].limit_err = pos - limot[i].histop;
                info->m++;
            }
        }
    }
...
dReal dJointGetLMotorPosition( dJointID j, int anum )
{
    dxJointLMotor* joint = ( dxJointLMotor* )j;
    dAASSERT( joint && anum >= 0 && anum < 3 );
    checktype( joint, LMotor );
    if ( anum < 0 ) anum = 0;
    if ( anum > 3 ) anum = 3;
    return joint->position[anum];
}

void dJointSetLMotorPosition( dJointID j, int anum, dReal position )
{
    dxJointLMotor* joint = ( dxJointLMotor* )j;
    dAASSERT( joint && anum >= 0 && anum < 3 );
    checktype( joint, LMotor );
