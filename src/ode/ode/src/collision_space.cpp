...
void dGeomMoved (dxGeom *geom)
{
    ...
    if (!parent && geom->parent_geom) {
        geom->gflags |= GEOM_DIRTY | GEOM_AABB_BAD;
        geom = geom->parent_geom;
        parent = geom->parent_space;
    }
...
void dxHashSpace::collide (void *data, dNearCallback *callback)
{
    ...
    //   the following line is causing clang warnings about undefined-bool-conversion
    //dAASSERT(this && callback);
    dAASSERT(this != NULL && callback);
...
dxSpace *dSimpleSpaceCreate_ST (dxSpace *space)
{
...
{
#ifdef ODE_MT
    dxSpace* newSpace = dSpaceCreate_MT(space, &dSimpleSpaceCreate_ST);
#else
    dxSpace* newSpace = dSimpleSpaceCreate_ST(space);
#endif
    return newSpace;
}

dxSpace *dHashSpaceCreate_ST (dxSpace *space)
{
    return new dxHashSpace (space);
}

dxSpace *dHashSpaceCreate_MT (dxSpace *space)
{
#ifdef ODE_MT
    return dSpaceCreate_MT(space, &dHashSpaceCreate_ST);
#else
    return dHashSpaceCreate_ST(space);
#endif
}
...
void dSpaceDestroy_ST (dxSpace *space)
{
    dAASSERT (space);
...
{
#ifdef ODE_MT
    dSpaceDestroy_MT(space, &dSpaceDestroy_ST);
#else
    dSpaceDestroy_ST(space);
#endif
}
...
void dSpaceAdd_ST (dxSpace *space, dxGeom *g)
{
    dAASSERT (space);
    dUASSERT (dGeomIsSpace(space),"argument not a space");
...
{
#ifdef ODE_MT
    dSpaceAdd_MT(space, g, &dSpaceAdd_ST);
#else
    dSpaceAdd_ST(space, g);
#endif
}
