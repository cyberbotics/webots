...
#ifdef ODE_MT
	dxClusteredWorldAndSpace* cw = dxClusterManager::instance()->currentCWAS;

	// only display the subworlds in the multithreading case if the main world is passed in arguments
	bool displaySubWorlds = cw && w == cw->originalWorld;
	int worldsCount = displaySubWorlds ? cw->clusterCount + 1 : 1;

	dWorldID *ww = (dWorldID *) malloc(worldsCount * sizeof(dWorldID));

	int k = 0;
	ww[k++] = w;
	if (displaySubWorlds) {
		for (int i = 0; i < cw->clusterCount; ++i)
			if (cw->worlds[i] != NULL)
				ww[k++] = cw->worlds[i];
	}

	dAASSERT(k == worldsCount);

#endif

#ifdef ODE_MT
	for (int i = 0; i < k; ++i) for (dxBody *b=ww[i]->firstbody; b; b=(dxBody*)b->next) {
#else
...
#ifdef ODE_MT
	for (int i = 0; i < k; ++i) for (dxJoint *j=ww[i]->firstjoint; j; j=(dxJoint*)j->next) {
#else
