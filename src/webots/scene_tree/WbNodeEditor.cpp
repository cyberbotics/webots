...
void WbNodeEditor::printUrl() {
  if (mNode->isProtoInstance()) {
    const QString &url = WbProtoManager::instance()->externProtoDeclaration(mNode->modelName());
    WbLog::info(
      tr("EXTERNPROTO URL of '%1': '%2'").arg(mNode->modelName()).arg(WbProtoManager::instance()->formatExternProtoPath(url)));
  }
}
...
void WbNodeEditor::update() {
  if (mNode && !mNode->isUseNode()) {
    ...
    if (mNode->isProtoInstance()) {
      ...
      const QString &url = WbProtoManager::instance()->externProtoDeclaration(mNode->modelName());
      mPrintUrl->setToolTip(WbProtoManager::instance()->formatExternProtoPath(url));
    } else
