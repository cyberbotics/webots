...
dGeomID WbMatter::createOdeGeomFromTransform(dSpaceID space, WbPose *pose) {
  assert(space);
...
void WbMatter::createOdeGeomFromInsertedShapeItem() {
  ...
  if (pose && pose->isInBoundingObject()) {
    insertedGeom = createOdeGeomFromTransform(upperSpace(), pose);
    if (insertedGeom)
...
dGeomID WbMatter::createOdeGeomFromGroup(dSpaceID space, WbGroup *group) {  // group is a *WbGroup but not a *WbPose
  ...
  while (it.hasNext()) {
    ...
    if (pose)
      createOdeGeomFromTransform(simpleSpace, pose);
    else {
...
dGeomID WbMatter::createOdeGeomFromNode(dSpaceID space, WbBaseNode *node) {
  ...
  // cppcheck-suppress knownConditionTrueFalse
  if (pose)
    return createOdeGeomFromTransform(space, pose);
