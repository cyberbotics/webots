    ...
    case C_SUPERVISOR_POSE_CHANGE_TRACKING_STATE: {
      ...
      if (enable) {
        ...
        if (fromNodeId && !fromTransformNode)
          mRobot->warn(tr("Pose tracking can be exclusively used with Transform (or derived) 'from_node' argument, but '%1' "
                          "(%2) is given. The absolute pose in global coordinates will be returned.")
