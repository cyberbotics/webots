...
void WbTemplateManager::regenerateNode(WbNode *node) {
  ...
  if (!newNode) {
    WbLog::error(tr("Template regeneration failed. The node cannot be generated."), false, WbLog::filterName(WbLog::PARSING));
    delete newNode;
    if (isWorldInitialized)
  ...
  if (node->isProtoParameterNode()) {
    ...
    if (parentGroup) {
      ...
      WbLog::error(tr("Template regeneration failed. Unsupported node type."), false, WbLog::filterName(WbLog::PARSING));
      delete newNode;
      emit abortNodeRegeneration();
      return;
    }
