...
QString WbUrl::computePath(const WbNode *node, const QString &field, const QString &rawUrl) {
  ...
  if (QDir::isRelativePath(url)) {
    ...
    QString alias = field;
    const WbField *pf;
    qDebug() << "STARTING NODE" << n->modelName();

    qDebug() << "FOLLOW ALIAS";
    const WbField *f = n->findField(field);
    while (n) {
      qDebug() << "  CHECKING NODE" << n->modelName() << "FIELD" << alias;
      f = n->findField(alias);
      if (f) {
        alias = f->alias();
        qDebug() << "  ALIAS NOW " << alias;
        n = n->parentNode();
        if (alias.isEmpty())
          break;
      } else
    ...
    qDebug() << "REACHED NODE " << n->modelName() << n->protoParameterNode();

    if (n->protoParameterNode()) {
      n = n->protoParameterNode();  // move to the parameter version

      while (n->isProtoParameterNode()) {  // consume any other proto parameter nodes defined in the header of a PROTO
        qDebug() << "  NODE" << n->modelName() << "IS A PROTO PARAMETER";
        n = n->parentNode();
        qDebug() << "  JUMP TO PARENT" << n->modelName();
      }
      // the process above will stop at the PROTO itself, since it's a param, the node we want is outside it
      assert(n->parentNode());
      n = n->parentNode();
    }

    qDebug() << "REACHED NODE " << n->modelName() << n->isProtoParameterNode() << n->isProtoDescendant();

    protoModel = WbNodeUtilities::findContainingProto(n);
    assert(protoModel);
    // qDebug() << "CONTAINER" << pm->name();
    // url = combinePaths(url, pm->url());

    qDebug() << "CHECK FOR DERIVED OF " << protoModel->name() << "? " << protoModel->isDerived();
    assert(protoModel);
    // if it happens to be a derived PROTO, then the search needs to be narrowed down to the ascendant that sets the field
    if (protoModel->isDerived() && !QFileInfo(combinePaths(url, protoModel->url())).exists()) {
      qDebug() << protoModel->name() << "IS DERIVED";
      ...
      for (int i = ancestry.size() - 1; i >= 0; --i) {
        // qDebug() << "ANC" << ancestry[i]->name() << ancestry[i]->parameterAliases();
    ...
    */
    ...
    qDebug() << "::::::::::::::::::::::::::::::::::::::\nSEARCHING URL: " << url;
    while (n) {
      qDebug() << "CHECKING NODE" << n->modelName();
      if (f) {
        alias = f->alias();
        n = n->parentNode();
      } else {  // either the 'IS' chain ended, or there wasn't a chain and the node was immediately internal to a PROTO
        protoModel = WbNodeUtilities::findContainingProto(n, true);
        qDebug() << "CONSIDERING PROTO MODEL" << protoModel->name() << protoModel->parameterAliases();
        // if (protoModel->parameterAliases().contains(alias)) {
        //  n = n->parentNode();
        //  protoModel = WbNodeUtilities::findContainingProto(n, true);
        //}
        break;
    ...
    // if it happens to be a derived PROTO, then the search needs to be narrowed down to the ascendant that sets the field
    if (protoModel->isDerived() && !QFileInfo(combinePaths(url, protoModel->url())).exists()) {
      qDebug() << protoModel->name() << "IS DER" << field;
      // reach the bottom of the ancestry by keeping track of all the members
      QVector<const WbProtoModel *> ancestry;
      ancestry << protoModel;
      while (protoModel->ancestorProtoModel()) {
    ...
    qDebug() << "==> DOING REL TO" << protoModel->name();
    url = combinePaths(url, protoModel->url());
    */
    //}
  }
