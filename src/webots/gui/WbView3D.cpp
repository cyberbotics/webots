...
void WbView3D::onSelectionChanged(WbAbstractPose *selectedAbstractPose) {
  assert(mWorld);

  if (mWorld->isCleaning())
    return;

  WbSolid *const selectedSolid = dynamic_cast<WbSolid *>(selectedAbstractPose);
...
void WbView3D::selectNode(const QMouseEvent *event) {
  ...
  const WbAbstractPose *const selectedPose = selection->selectedAbstractPose();
  ...
  if (isContextMenuShortcut(event))
    ...
    const WbMatter *const previousTopMatter =
      selectedPose != NULL ? WbNodeUtilities::findUppermostMatter(selectedPose->baseNode()) : NULL;
    ...
    if (visiblePickedMatter == selectedPose) {
      if (alt)
      ...
      if (topMatter == visiblePickedMatter) {
        ...
        selection->confirmSelectedPoseFromView3D();
        return;
      }
