...
if (rate <= 0) || (point_cloud_enable == false)
    points = []
    if (rate <= 0)
        calllib('libController', 'wb_console_print', 'Warning: wb_lidar_get_point_cloud() called for a disabled lidar! Please call wb_lidar_enable() before.', 1);
    else
        calllib('libController', 'wb_console_print', 'Warning: wb_lidar_get_point_cloud() called for a lidar with point cloud disabled! Please call wb_lidar_enable_point_cloud() before.', 1);
    end
else
