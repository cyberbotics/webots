...
// TODO: Check the Youbot for the use of omniwheels
...
int main(int argc, char **argv) {
  ...
  const int urg04lx_width =
      wb_lidar_get_horizontal_resolution(urg04lx); // 667 points
  ...
  double front_obstacle = 0.0, front_left_obstacle = 0.0,
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    if (left_obstacle > right_obstacle &&
        left_obstacle > NEAR_OBSTACLE_THRESHOLD) {
      const double speed_factor = (1.0 - FAST_DECREASE_FACTOR * left_obstacle) *
                                  MAX_SPEED / left_obstacle;
      left_speed = check_speed(speed_factor * left_obstacle);
      right_speed = check_speed(speed_factor * right_obstacle);

    } else if (front_left_obstacle > front_right_obstacle &&
               front_left_obstacle > FAR_OBSTACLE_THRESHOLD) {
      const double speed_factor =
          (1.0 - SLOW_DECREASE_FACTOR * front_left_obstacle) * MAX_SPEED /
          front_left_obstacle;
      left_speed = check_speed(speed_factor * front_left_obstacle);
      right_speed = check_speed(speed_factor * front_right_obstacle);

    } else if (front_right_obstacle > front_left_obstacle &&
               front_right_obstacle > FAR_OBSTACLE_THRESHOLD) {
      const double speed_factor =
          (1.0 - SLOW_DECREASE_FACTOR * front_right_obstacle) * MAX_SPEED /
          front_right_obstacle;
      left_speed = check_speed(speed_factor * front_left_obstacle);
      right_speed = check_speed(speed_factor * front_right_obstacle);

    } else if (right_obstacle > left_obstacle &&
               right_obstacle > NEAR_OBSTACLE_THRESHOLD) {
      const double speed_factor =
          (1.0 - FAST_DECREASE_FACTOR * right_obstacle) * MAX_SPEED /
          right_obstacle;
      left_speed = check_speed(speed_factor * left_obstacle);
      right_speed = check_speed(speed_factor * right_obstacle);
      ...
      const double speed_factor =
          (1.0 - FAST_DECREASE_FACTOR * front_obstacle) * MAX_SPEED /
          front_obstacle;
      // more obstacles on the right, so make a left u-turn to avoid being stuck
      if (front_right_obstacle > front_left_obstacle ||
          right_obstacle > left_obstacle) {
