...
static void wait_until_target_position(WbDeviceTag inertial_unit, const double roll_target, const double pitch_target, const double yaw_target, const int maxSteps) {
  int steps = maxSteps;
...
int main(int argc, const char *argv[]) {
  ...
  printf("Lock vacuum cup\n");
  wb_vacuum_cup_lock(vaccum_cup);
  ...
  for (int i = 0; i < NB_STEPS; i++) {
    ...
    if (i == 12) {
      printf("Unlock vacuum cup\n");
      wb_vacuum_cup_unlock(vaccum_cup);
    }
  }
