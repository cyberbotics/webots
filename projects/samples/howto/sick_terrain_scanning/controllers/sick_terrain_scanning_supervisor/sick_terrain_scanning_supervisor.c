  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    for (i = 0; i < laser_width; i++) {
      if (buffer[i] > 25)  // range up to 25 metres
        buffer[i] = 25;
      pxfill[i] = width * (translation[X] + GROUND_X / 2 + sin(rotation[3] + (laser_width - i) * pi / 180) * buffer[i]) / GROUND_X;
      pyfill[i] = height * (-translation[Y] + GROUND_Y / 2 + cos(rotation[3] + (laser_width - i) * pi / 180) * buffer[i]) / GROUND_Y;
    }
