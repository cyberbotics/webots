...
static WallFollowingMetric *
generate_metric_from_proto(const double *robot_starting_position) {
  ...
  if (angles) {
    ...
    WbFieldRef translation_field =
        wb_supervisor_node_get_field(wall, "translation");
    ...
    double first_segment_orientation =
        wb_supervisor_field_get_sf_rotation(rotation_field)[3];

    // Recovers the size of one wall segment.
    double segment_length = wb_supervisor_field_get_sf_float(length_field);

    metric = create_new_wall_following_metric(
        robot_starting_position, first_segment_position, angles, nb_angles,
...
int main(int argc, char **argv) {
  ...
  if (metric) {
    ...
    while (wb_robot_step(time_step) != -1 &&
           wb_robot_get_time() < BENCHMARK_DURATION) {
      position = wb_supervisor_node_get_position(pioneer);
      position2d[0] = position[2];
      position2d[1] = position[0];
      ...
      PathSegment *current_segment =
          path_get_closest_segment(metric->path, position2d, &distance);

      sprintf(str_buffer, "Distance from path: %f", distance);
      wb_supervisor_set_label(0, str_buffer, 0.01, 0.01, 0.05, 0x0000ff, 0.0,
                              "Lucida Console");

      sprintf(str_buffer, "Current segment: %d", current_segment->index);
      wb_supervisor_set_label(1, str_buffer, 0.01, 0.04, 0.05, 0x0000ff, 0.0,
                              "Lucida Console");
#endif
      ...
      wb_supervisor_set_label(2, str_buffer, 0.01, 0.07, 0.05, 0x0000ff, 0.0,
                              "Lucida Console");

      sprintf(str_buffer, "Current time: %f", wb_robot_get_time());
      wb_supervisor_set_label(3, str_buffer, 0.01, 0.1, 0.05, 0x0000ff, 0.0,
                              "Lucida Console");
#endif
    ...
    do {
      ...
      while ((answer_message = wb_robot_wwi_receive_text())) {
        if (strncmp(answer_message, "record:", 7) == 0) {
          robotbenchmark_record(answer_message, "wall_following",
                                metric->performance);
          waiting_answer = 0;
